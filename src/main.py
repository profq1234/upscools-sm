...
def send_pin_to_pinterest(image_paths, title, description, destination_link):
    ...
    try:
        ...
        # Handle Single vs Carousel (Multiple Images)
    ...
    for item in data:
        ...
        if captions_dict:
            ...
            for format_key, image_paths in generated_formats.items():
                ...
                if ENABLE_PINTEREST:
                    print(f"Sending {short_format_key} Carousel to Pinterest...")
                    pin_success = send_pin_to_pinterest(image_paths, meta_title, full_caption, destination_url)
                    if not pin_success: all_formats_success = False

                for img in image_paths:
                    if os.path.exists(img): os.remove(img)

        # ==========================================
        # WORKFLOW B: OLD DATA (Hybrid Workflow)
        # ==========================================
        else:
            print("🟡 OLD DATA DETECTED: Triggering hybrid workflow (Telegram: Combined, Pinterest: Separate)...")
            all_image_paths = []
            ...
            if ENABLE_PINTEREST:
                print("Sending images to Pinterest as 4 separate Carousels...")
                for format_key, image_paths in generated_formats.items():
                    short_format_key = f"{format_key.split('_')[0]}_{format_key.split('_')[1]}"
                    print(f"Sending {short_format_key} Carousel to Pinterest...")
                    pin_success = send_pin_to_pinterest(image_paths, meta_title, full_caption, destination_url)
